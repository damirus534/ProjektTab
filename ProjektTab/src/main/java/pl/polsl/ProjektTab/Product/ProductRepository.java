...
public interface ProductRepository extends JpaRepository<Product, Long> {
        @Query("SELECT new pl.polsl.ProjektTab.Filters.ReturnValue(c.id,c.description,c.productName,p.photoUrl) FROM ProductInfo c JOIN c.photos p ON  c.id=p.productInfo.id group by c.id")
        public List<ReturnValue> getSeperatedProduct();


        @Query("SELECT new pl.polsl.ProjektTab.Filters.ReturnValue(c.id,c.description,c.productName,p.photoUrl) FROM ProductInfo c JOIN c.photos p ON  c.id=p.productInfo.id AND c.id=?1 group by c.id")
